...
3. Call `start_run` exactly once for that attempt with `openBrowser: true`. It starts or reuses `dsh web`, opens the page, creates a workspace-bound Web session, and submits the task. Tell the user the returned `webUrl`. A timeout or delayed response is not permission to start a duplicate run.
...
7. Call `cancel_run` only when the user replaces the task, the run is clearly stuck, or continuing would be unsafe. This cancels the agent turn but leaves the Web page available. Use `stop_service` only when the local server is no longer needed.

## Safety

- DeepSeek Harness Web starts as a loopback-only local child process and receives access to the requested workspace. Do not pass a broader directory than needed.
...
- Each run is a visible Web session. Give correction feedback through a new run against the same workspace, and keep the original page available for audit.
